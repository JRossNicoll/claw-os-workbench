import { Github, Hexagon, MessageSquare, Key, Check, Hash, MessageCircle } from "lucide-react";
...
const integrations = [
  ...
  { id: "discord", name: "Discord", description: "Send alerts and notifications to Discord channels", icon: MessageCircle },
  { id: "slack", name: "Slack", description: "Integrate with Slack workspaces for team notifications", icon: Hash },
...
export function StepIntegrations({ selected, onToggle }: StepIntegrationsProps) {
  return (
    <div>
      ...
      <div className="space-y-1.5">
        {integrations.map((item) => {
          const isConnected = selected.includes(item.id);
          return (
            <button key={item.id} onClick={() => onToggle(item.id)} className={cn("w-full flex items-center gap-3.5 p-3.5 rounded-lg surface-elevated text-left transition-all duration-200", isConnected ? "border-primary/40" : "hover:border-primary/15")}>
